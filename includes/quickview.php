...
add_action('wp_footer', function(){
  ...
  <div id="annesfs-qv" class="annesfs-qv" style="display:none">
    ...
    <div class="annesfs-qv__panel" role="dialog" aria-modal="true" aria-labelledby="annesfs_qv_title">
      <button type="button" class="annesfs-qv__close" aria-label="Close">×</button>
      <div class="annesfs-qv__content">
        <div class="annesfs-qv__loading">Loading…</div>
      </div>
    </div>
  </div>
  <?php
...
function annesfs_qv(){
  ...
  if ( ! $pid ) wp_send_json_error(['html'=>'<p>Invalid product.</p>']);
  $product = wc_get_product($pid);
  if ( ! $product ) wp_send_json_error(['html'=>'<p>Product not found.</p>']);
  ob_start(); ?>
  <div class="qv-head">
    <div class="qv-title" id="annesfs_qv_title"><?php echo esc_html($product->get_name()); ?></div>
    <div class="qv-price"><?php echo wp_kses_post($product->get_price_html()); ?></div>
  </div>
  <div class="qv-body">
    <div class="qv-media">
      <?php echo $product->get_image( 'woocommerce_single', ['style'=>'border-radius:10px;max-width:100%;height:auto;object-fit:cover'] ); ?>
    </div>
    <div class="qv-form">
      <?php
